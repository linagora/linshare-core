...
public class FunctionalityFacadeImpl implements FunctionalityFacade {
	...
	public List<PolicyVo> getAllActivationPolicy(String domainIdentifier) {
		...
		List<Functionality> list = functionalityService
				.getAllAvailableFunctionalities(domainIdentifier);
		logger.debug("AllAvailableFunctionalities : " + list.size());
		for (Functionality functionality : list) {
			res.add(new PolicyVo(functionality.getActivationPolicy(),
					functionality.getIdentifier(), domainIdentifier));
		}
	...
	public List<PolicyVo> getAllConfigurationPolicy(String domainIdentifier) {
		List<PolicyVo> res = new ArrayList<PolicyVo>();
		List<Functionality> list = functionalityService
				.getAllAlterableFunctionalities(domainIdentifier);
		logger.debug("AllAlterableFunctionalities : " + list.size());
		for (Functionality functionality : list) {
			res.add(new PolicyVo(functionality.getConfigurationPolicy(),
					functionality.getIdentifier(), domainIdentifier));
		}
	...
	public List<FunctionalityVo> getAllParameters(String domainIdentifier) {
		...
		List<Functionality> list = functionalityService
				.getAllEditableFunctionalities(domainIdentifier);
		...
		for (Functionality functionality : list) {
			if (functionality.getType().equals(FunctionalityType.INTEGER)) {
				...
				res.add(new IntegerValueFunctionalityVo(functionality
						.getIdentifier(), domainIdentifier, f.getValue()));
				...
				res.add(new StringValueFunctionalityVo(functionality
						.getIdentifier(), domainIdentifier, f.getValue()));
				...
				if (f.getUnit().getUnitType().equals(UnitType.TIME)) {
					...
					res.add(new TimeValueFunctionalityVo(functionality
							.getIdentifier(), domainIdentifier, f.getValue(),
							timeUnit.getUnitValue()));

				} else if (f.getUnit().getUnitType().equals(UnitType.SIZE)) {
					FileSizeUnitClass sizeUnit = (FileSizeUnitClass) f
							.getUnit();
					res.add(new SizeValueFunctionalityVo(functionality
							.getIdentifier(), domainIdentifier, f.getValue(),
							sizeUnit.getUnitValue()));

				} else {
					logger.error("Unknown Unit Functionality Type for : "
							+ functionality.getIdentifier());
				}
			} else if (functionality.getType().equals(
					FunctionalityType.UNIT_BOOLEAN_TIME)) {
				UnitBooleanValueFunctionality f = (UnitBooleanValueFunctionality) functionality;

				if (f.getUnit().getUnitType().equals(UnitType.TIME)) {
					TimeUnitClass timeUnit = (TimeUnitClass) f.getUnit();
					res.add(new TimeValueBooleanFunctionalityVo(functionality
							.getIdentifier(), domainIdentifier, f.getValue(),
							timeUnit.getUnitValue(), f.getBool()));
				} else {
					logger.error("Unknown Unit boolean Functionality Type for : "
							+ functionality.getIdentifier());
				}
			} else {
				logger.error("Unknown Functionality Type for : "
						+ functionality.getIdentifier() + " : "
						+ functionality.getType().toString());
			}
		}
	...
	@Override
	public void updateActivationPolicies(UserVo actorVo, List<PolicyVo> policies)
			throws BusinessException {
		if (isAuthorized(actorVo)) {
			for (PolicyVo policyVo : policies) {
				logger.debug("Domain: " + policyVo.getDomainIdentifier()
						+ " : " + "Func: "
						+ policyVo.getFunctionalityIdentifier() + " : "
						+ policyVo.getPolicy() + " : status : "
						+ policyVo.getStatus());
				Functionality f = functionalityService
						.getFunctionalityByIdentifiers(
								policyVo.getDomainIdentifier(),
								policyVo.getFunctionalityIdentifier());
	...
	@Override
	public void updateConfigurationPolicies(UserVo actorVo,
			List<PolicyVo> policies) throws BusinessException {
		if (isAuthorized(actorVo)) {
			for (PolicyVo policyVo : policies) {
				logger.debug("Domain: " + policyVo.getDomainIdentifier()
						+ " : " + "Func: "
	...
	@Override
	public void updateParameters(UserVo actorVo,
			List<FunctionalityVo> functionalities) throws BusinessException {
		if (isAuthorized(actorVo)) {
			for (FunctionalityVo functionalityVo : functionalities) {
				Functionality entity = functionalityService
						.getFunctionalityByIdentifiers(
								functionalityVo.getDomainIdentifier(),
								functionalityVo.getIdentifier());
				entity.updateFunctionalityValuesOnlyFromVo(functionalityVo);
				functionalityService.update(
						functionalityVo.getDomainIdentifier(), entity);
			}
		}
	}
