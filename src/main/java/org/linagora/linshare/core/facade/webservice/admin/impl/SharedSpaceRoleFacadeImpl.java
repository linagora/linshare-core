...
public class SharedSpaceRoleFacadeImpl extends AdminGenericFacadeImpl implements SharedSpaceRoleFacade {
	...
	public SharedSpaceRole find(String uuid) throws BusinessException {
		...
		Validate.notEmpty(uuid, "The shared space role  uuid to find must be set.");
		return ssRoleService.find(authUser, authUser, uuid);
	}

	@Override
	public List<SharedSpaceRole> findAll() throws BusinessException {
		Account authUser = checkAuthentication(Role.SUPERADMIN);
		return ssRoleService.findAll(authUser, authUser);
	}
