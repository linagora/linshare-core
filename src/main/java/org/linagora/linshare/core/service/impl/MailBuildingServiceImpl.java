...
public class MailBuildingServiceImpl implements MailContentBuildingService {
	
	private final static Logger logger = LoggerFactory.getLogger(MailBuildingServiceImpl.class);
	 ...
	 */
	public MailTemplate getMailTemplate(User actor, Language language, MailTemplateEnum mailTemplate) throws BusinessException {
		if (mailTemplate.equals(MailTemplateEnum.FOOTER)) {
			return new MailTemplate(getMailFooter(actor, language));
		} else {
			return new MailTemplate(getMailContent(actor, language,
					MailContentType.adapt(mailTemplate)));
		}
		
//		Set<MailTemplate> templates = actor.getDomain().getMessagesConfiguration().getMailTemplates();
//		
//		
//		Set<MailContentLang> mailContents = actor.getDomain()
//				.getCurrentMailConfiguration().getMailContents();
//		
//		for (MailContentLang mailContentLang : mailContents) {
//			if (language.equals(mailContentLang.getLanguage())) {
//				if (mailTemplate.equals(mailContentLang.getMailContent().getMailContentType())) {
//					// c'est bon
//				}
//			}
//		}
...
//			if (mailTemplate_.getLanguage().equals(language)
//					&& mailTemplate_.getMailTemplate().equals(mailTemplate)) {
//				return new MailTemplate(mailTemplate_);
//			}
//		}

//		logger.error("Bad mail template " + mailTemplate.name()
//				+ " for language " + language.name());
//		throw new TechnicalException(TechnicalErrorCode.MAIL_EXCEPTION,
//				"Bad mail template " + mailTemplate.name() + " for language "
//						+ language.name());
	}
	 ...
	 */
	private MailContent getMailContent(User actor, Language language, MailContentType mailContentType) throws BusinessException {
//		/*
//		 * search domains recursively
//		 */
//		MailContent content;
//		AbstractDomain domain = actor.getDomain();
//
...
//		} while (domain != null && content == null);
		...
		if (content == null)
			throw new BusinessException("no mailconfig for domain : "
	 ...
	 * @return 
	 */
	private MailFooter getMailFooter(User actor, Language language) {
		MailConfig cfg = actor.getDomain().getCurrentMailConfiguration();
		MailFooterLang lang = cfg.getMailFooters().get(language.toInt());
		
		return lang.getFooter();
	}
...
//		MailTemplate greetings = buildTemplateGreetings(sender, mailContainer.getLanguage(), recipient);
	...
	private MailContainerWithRecipient buildMailContainerSetProperties(User sender, final MailContainerWithRecipient inputMailContainer, String personalMessage, MailTemplate greetings) throws BusinessException {
		...
		String mailLayoutTxt = domain.getCurrentMailConfiguration().getMailLayoutText().getLayout();
		String mailLayoutHtml = domain.getCurrentMailConfiguration().getMailLayoutHtml().getLayout();
				
		
		if (displayLogo) {
			mailLayoutHtml = mailLayoutHtml.replace("%{image}", "<img src=\"cid:image.part.1@linshare.org\" /><br/><br/>");
		} else {
			//no logo in html
			mailLayoutHtml = mailLayoutHtml.replace("%{image}", "");
		}
		...
		if (personalMessage != null && personalMessage.trim().length() > 0) {
			...
			mailLayoutTxt = StringUtils.replace(mailLayoutTxt, "${personalMessage}", personalMessageTemplate.getContentTXT());
	        mailLayoutHtml = StringUtils.replace(mailLayoutHtml, "${personalMessage}", personalMessageTemplate.getContentHTML());
			mailLayoutTxt = StringUtils.replace(mailLayoutTxt, "%{personalMessage}", personalMessageTemplate.getContentTXT());
	        mailLayoutHtml = StringUtils.replace(mailLayoutHtml, "%{personalMessage}", personalMessageTemplate.getContentHTML());
		}
		else {
			mailLayoutTxt = StringUtils.replace(mailLayoutTxt, "${personalMessage}", "");
	        mailLayoutHtml = StringUtils.replace(mailLayoutHtml, "${personalMessage}", "");
			mailLayoutTxt = StringUtils.replace(mailLayoutTxt, "%{personalMessage}", "");
	        mailLayoutHtml = StringUtils.replace(mailLayoutHtml, "%{personalMessage}", "");
		}
...
//        mailLayoutTxt = StringUtils.replace(mailLayoutTxt, "%{greetings}", greetings.getContentTXT());
        ...
        mailLayoutHtml = StringUtils.replace(mailLayoutHtml, "${greetings}", greetings.getContentHTML());
        mailLayoutHtml = StringUtils.replace(mailLayoutHtml, "${footer}", footer.getContentHTML());
        mailLayoutHtml = StringUtils.replace(mailLayoutHtml, "${body}", inputMailContainer.getContentHTML());
        mailLayoutHtml = StringUtils.replace(mailLayoutHtml, "${mailSubject}", inputMailContainer.getSubject());
        ...
        mailContainer.setContentTXT(mailLayoutTxt);
        mailContainer.setContentHTML(mailLayoutHtml);
