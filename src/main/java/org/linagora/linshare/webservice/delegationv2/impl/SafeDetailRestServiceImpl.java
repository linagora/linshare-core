...
public class SafeDetailRestServiceImpl extends WebserviceBase implements
	...
	public SafeDetail delete(
			@ApiParam(value = "The actor (user) uuid.", required = true) @PathParam("actorUuid") String actorUuid,
			@ApiParam(value = "The safeDetail uuid.", required = true) @PathParam("uuid") String uuid)
			throws BusinessException {
		return safeDetailFacade.delete(actorUuid, uuid);
	}
	...
	public SafeDetail delete(
			@ApiParam(value = "The actor (user) uuid.", required = true) @PathParam("actorUuid") String actorUuid,
			@ApiParam(value = "The safeDetail to delete.", required = true) SafeDetail safeDetail)
			throws BusinessException {		// TODO Auto-generated method stub
		return safeDetailFacade.delete(actorUuid, safeDetail);
	}
	...
	@Path("/")
	@GET
	@ApiOperation(value = "Get all safeDetails.", response = SafeDetail.class, responseContainer = "Set")
	@ApiResponses({
			@ApiResponse(code = 403, message = "Current logged in account does not have the delegation role."),
			@ApiResponse(code = 400, message = "Bad request : missing required fields."),
			@ApiResponse(code = 500, message = "Internal server error."), })
	@Override
	public SafeDetail findAll(
			@ApiParam(value = "The actor (user) uuid.", required = true) @PathParam("actorUuid") String actorUuid)
			throws BusinessException {
