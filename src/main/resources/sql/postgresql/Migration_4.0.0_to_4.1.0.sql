...
-- Update UPLOAD_REQUEST_REMINDER
UPDATE mail_content SET subject='[(#{subject(${requestOwner.firstName}, ${requestOwner.lastName})})]',body='<!DOCTYPE html>
<html xmlns:th="http://www.thymeleaf.org">
   <head data-th-replace="layout :: header"></head>
   <body>
...
UPDATE policy SET status=false,default_status=false, system=true, policy=2 WHERE id IN (SELECT policy_activation_id FROM functionality WHERE identifier = 'WORK_GROUP__FILE_EDITION');
