                  ...
                  <tr>
                    <td align="center" bgcolor="#ffffff"
                      ...
                      <div align="center" style="text-align:center">
                        <a title="LinShare homepage" target="_blank"   style="border:0;line-height:100%;outline:none;text-decoration:none;width:198px;height:49px;padding:20px 0 20px 0">
                          <img 
th:src="#{logoLinShareBase64}"
style="display:inline-block;margin-bottom:20px;margin-top:20px"  width="198" alt="Logo LinShare english" height="49" />
                        </a>
                      </div>
                    </td>
                  </tr>
                      ...
                      <table border="0" cellpadding="20" cellspacing="0" width="100%">
                        ...
                        <tr>
                          <td style="border-collapse:collapse;padding:0px" valign="top">
                            <div align="left" style="color:#505050;font-family:Helvetica,Arial;font-size:14px;line-height:150%;text-align:left">
                                                
                          <th:block th:replace="${upperMainContentArea}" />
...
width="20" height="9" border="0" style="display:block;" alt="down arrow"/></td>
                          ...
                          <th:block th:replace="${bottomSecondaryContentArea}" />
                                    </div>
                                  </div>
                                </td>
                              </tr>
...
<!--/* <div data-th-insert="footer :: footer"> */-->
  <p                         style="line-height:15px;font-weight:300;margin-bottom:0;color:#b2b2b2;font-size:10px;margin-top:0">
                                You are using the Open Source and free version of
                                <a href="" style="text-decoration:none;color:#b2b2b2;"><strong>LinShare</strong>™</a>, powered by <a href="" style="text-decoration:none;color:#b2b2b2;"><strong>Linagora</strong></a> ©&nbsp;2009–2015.Contribute to
...
<span style="font-weight:bold;" data-th-text="${titleInfo}" >Download link title  </span>
  <br/>
<a target="_blank" style="color:#1294dc;text-decoration:none;" title="titleInfo"
...
<div  data-th-fragment="infoFileLinksListingArea(titleInfo,arrayFileLinks,isAnonymous)">
     <span style="font-weight:bold;" data-th-text="${titleInfo}" >Shared the </span>
...
<html xmlns:th="http://www.thymeleaf.org">
  ...
  <body>
    ...
    <section id="secondary-content">
      ...
      <th:block data-th-replace="layout :: infoFileLinksListingArea(#{common.filesInShare},${shares},${!actionDate})"/>
...
subject =  {0} a téléchargé {1}
mainMsgExt = Le destinataire externe <b>{0}</b> a téléchargé votre fichier :', 'downloadDate = Downloaded the
...
INSERT INTO mail_content (id, domain_abstract_id, description, visible, mail_content_type, subject, body, uuid, creation_date, modification_date, readonly, messages_french, messages_english) VALUES (9, 1, 'New sharing', true, 8, '[# th:if="${#strings.isEmpty(customSubject)}"]
  ...
  [# th:if="${anonymous}"]   
       [# th:if="${sharesCount} >=  1"] 
           [( #{subjectPlural(${shareOwner.mail})})]
       [/]
        [# th:if="${sharesCount} ==  1"] 
            [( #{subjectSingular(${shareOwner.mail})})]
       [/]
  [/]
[/]
  ...
  [/]
  [# th:if="${anonymous}"]
      [(${customSubject})]  [( #{subjectCustomAlt(${shareOwner.mail})})]
  [/]
[/]', '<!DOCTYPE html>
<html xmlns:th="http://www.thymeleaf.org">
  <head  data-th-replace="layout :: header"></head>
  <body>
    ...
    <section id="main-content">
      ...
      <div   th:assert="${!#strings.isEmpty(customMessage)}" th:replace="layout :: contentMessageSection( ~{::#message-title}, ~{::#message-content})">
      ...
      <div th:replace="layout :: contentUpperSection( ~{::#section-content})">
        <div id="section-content">
          ...
          <!--/* Main email  message content*/-->
          <p>
            <span  data-th-if="(${sharesCount} ==  1)" data-th-utext="#{mainMsgSingular(${shareOwner.firstName},${shareOwner.lastName},${sharesCount})}">
            Peter WILSON has shared 4 file with you 
            </span>
            <span  data-th-if="(${sharesCount} >=  1)"  data-th-utext="#{mainMsgPlural(${shareOwner.firstName},${shareOwner.lastName},${sharesCount})}">
            Peter WILSON has shared 4 files with you 
            </span>
            <br/>
            <!--/* Checks if the external user has a password protected file share */-->
            <span data-th-if="(${protected})">
            <span data-th-text="#{helpPasswordMsg}" ></span>
            </span>
...
</html>', '250e4572-7bb9-4735-84ff-6a8af93e3a42', now(), now(), true, 'msgFrom=Vous avez un message de 
downloadBtn = Téléchargé
...
subjectSingular =  {0} vous a partagé un fichier
subject= {0} vous a partagé des fichiers
...
helpPasswordMsg =Cliquez sur le lien pour le télécharger et saisissez le mot de passe ci-dessous.', 'msgFrom=You have a message from 
...
password = Password
helpPasswordMsg =Click on the link below in order to download them and enter the following password.');
...
<html xmlns:th="http://www.thymeleaf.org">
  <body>
    <div data-th-fragment="email_footer">
                                <td style="border-collapse:collapse;padding: 4px 0 0 17px;" valign="top">
                                  <p style="margin: 0; font-size: 9px;"><span th:text="#{linkToProductPage}">En savoir plus sur</span> <a  href="http://www.linshare.org/" title="LinShare homepage" target="_blank"style="text-decoration:none; color:#a9a9a9;"><strong th:text="#{productName}">LinShare</strong>™</a>
                                  </p>
                                </td>
                                <td style="border-collapse:collapse; padding: 3px 17px 0 0" valign="top" width="60">
...
src=''data:image/png;base64, ''
 ...
 </html>', now(), now(), 'e85f4a22-8cf2-11e3-8a7a-5404a683a462', true, 'linkToProductPage=En savoir plus sur', 'linkToProductPage=Learn more about');
