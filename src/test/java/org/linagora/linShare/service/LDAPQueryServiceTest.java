	...
	public void setUp() throws Exception {
		if (!initialized) {
			Parameter param = new Parameter("testParam", new Long(100L), new Long(100L), new Long(1000L), new Long(0L), Boolean.FALSE, Boolean.FALSE, Boolean.FALSE, Boolean.FALSE, 
					...
					new HashSet<MailTemplate>(), new HashSet<MailSubject>());
...
//		fail("Not yet implemented");
	...
	public void testAuth() throws BusinessException, NamingException, IOException {
		User user = ldapQueryService.auth("user1", "password1", DOMAIN_IDENTIFIER);
		Assert.assertNotNull(user);
		user = ldapQueryService.auth("user1", "bla", DOMAIN_IDENTIFIER);
		Assert.assertNull(user);
		user = ldapQueryService.auth("user1@linpki.org", "password1", DOMAIN_IDENTIFIER);
		Assert.assertNotNull(user);
	}
