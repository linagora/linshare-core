...
public class ComputeDocumentExpirationBatchImplTest extends AbstractTransactionalJUnit4SpringContextTests {

	@Qualifier("computeDocumentExpirationBatch")
	@Autowired
	private GenericBatch computeDocumentExpirationBatch;

	@Before
	public void setUp() throws Exception {
		logger.debug(LinShareTestConstants.BEGIN_SETUP);
		this.executeSqlScript("import-tests-batches-accounts.sql", false);
		this.executeSqlScript("import-tests-check-nullExpiredEntry.sql", false);
	...
	public void testBatch() throws BusinessException, JobExecutionException {
		...
		batches.add(computeDocumentExpirationBatch);
	...
	public void testBatchExecution() throws BusinessException, JobExecutionException {
		List<String> l = computeDocumentExpirationBatch.getAll();
		Context c;
		int count = 0;
		for (int i = 0; i < l.size(); i++) {
			c = computeDocumentExpirationBatch.execute(l.get(i), l.size(), i);
			if (c != null) {
				count++;
			}
		}
